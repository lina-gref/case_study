 ...
 *    Used by: ChatPage.requestImageGeneration() instead of raw selectors
 *    Benefit: If UI changes, we update ONE file, not all tests
 * 
 * ✅ **DTOs** - Validate backend contract, not just response existence
 *    Used by: ImageAssetDTO to enforce schema and validate provider
 *    Benefit: Catches breaking changes in API contract immediately
 * 
 * ✅ **Factories** - Don't hardcode test data
 *    Used by: UserFactory.create('premium')
 *    Benefit: Adding new user types = 3 lines in factory
 * 
 * ✅ **Separation of Concerns** - Each component has ONE responsibility
 *    ChatPage = UI interactions only
 *    ImageAssetDTO = Response validation only
 *    UserFactory = Test data only
 * 
 * ✅ **Security/Cost Control** - Tests verify business rules
 *    Used by: imageAsset.isFromAuthorizedProvider()
 *    Benefit: Prevents unauthorized backend services from being used
 * 
 * What This Demonstrates to the Interviewer:
 * ✅ I understand backend contract testing
 * ✅ I know how to validate API responses type-safely
 * ✅ I think about business requirements (cost control, security)
 * ✅ I build systems that catch integration issues early
